...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = \
        'Plays media consisted of .rgb video file and .wav audio file')
    parser.add_argument('rgb', metavar='rgb', help='Path to .rgb file')
    parser.add_argument('wav', metavar='wav', help='Path to .wav file')
    parser.add_argument('-width', type=int, metavar='', \
        default=480, help='width of video frame')
    parser.add_argument('-height', type=int, metavar='', \
        default=270, help='height of video frame')
    parser.add_argument('-fps', type=int, metavar='', \
        default=30, help='frames per second')
    parser.add_argument('-buf', type=int, metavar='', \
        default=300, help='buffer length')
    parser.add_argument('-wsr', type=int, metavar='', \
        default=48000, help='.wav sampling rate')
    args = parser.parse_args()

    app = QApplication(sys.argv)
    player = Player(args.rgb, args.wav, args.width, args.height, args.fps, args.buf, args.wsr)
    sys.exit(app.exec_())
    player.rgb_f.close()
